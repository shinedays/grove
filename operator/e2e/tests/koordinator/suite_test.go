...
// The suite connects to an existing cluster (no cluster creation, no teardown) that has
// Koordinator installed and Grove deployed with the koord-scheduler profile — see the
// package documentation in setup.go for the exact prerequisites. On clusters without
// Koordinator, every test skips itself via skipUnlessKoordinator.
