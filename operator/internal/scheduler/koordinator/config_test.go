...
func TestParseConfig_TopologyKeyMappings_ArbitraryLayerNames(t *testing.T) {
	// Layer names are administrator-defined in the Koordinator ClusterNetworkTopology CR
	// (plus the built-in NodeTopologyLayer), so any non-empty value must be accepted.
	raw := configv1alpha1.KoordinatorSchedulerConfiguration{
		TopologyKeyMappings: map[string]string{
			"mycompany.com/rack":     "rackLayer",
			"mycompany.com/spine":    "spineLayer",
	...
	assert.Equal(t, "spineLayer", cfg.topologyKeyMappings["mycompany.com/spine"])
	assert.Equal(t, "NodeTopologyLayer", cfg.topologyKeyMappings["kubernetes.io/hostname"])
}

func TestParseConfig_TopologyKeyMappings_EmptyLayerRejected(t *testing.T) {
	raw := configv1alpha1.KoordinatorSchedulerConfiguration{
		TopologyKeyMappings: map[string]string{
			"mycompany.com/dc": "",
		},
...
func TestParseConfig_ErrorOnLaterFieldReturnsStrictDefaults(t *testing.T) {
	// GangMode is valid and would be applied before DefaultQoSClass fails validation.
	timeout := int32(90)
	raw := configv1alpha1.KoordinatorSchedulerConfiguration{
